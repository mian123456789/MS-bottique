...
const emptyState: FactoryState = {
  lots: [], sizes: [], variants: [], records: {}, warehouse: [], receipts: [], dispatches: [], gatepasses: [],
...
function LotProgress({ state, setModal }: PageProps) {
  ...
  return <div className="page-stack"><SectionHead eyebrow="CONTROL TOWER" title="Lot Progress / Production Tracking" detail="One live view of every design, quantity and department handoff." action={<div className="action-group"><button className="button secondary" onClick={() => exportRows(rows, "MS-Boutique-Lot-Progress")}><Download size={16} /> Excel</button><button className="button secondary" onClick={() => printLotBook(rows, state, "Lot Progress Full Sheets")}><FileText size={16} /> PDF</button><button className="button secondary" onClick={() => printLotBook(rows, state, "Lot Progress Full Sheets")}><Printer size={16} /> Print</button></div>} />
    ...
    <article className="panel table-panel tracking-table"><div className="panel-head compact"><span><b>{rows.length}</b> production lots</span><span className="updated-label"><span className="live-dot" /> Live quantities · Updated just now</span></div><div className="table-scroll"><table><thead><tr><th>Lot / Design</th><th>Fabrication</th><th>QTY</th><th>Department / Status</th><th>Completed / Pending</th><th>Required Date</th><th>Remarks</th><th className="right">Actions</th></tr></thead><tbody>{rows.map((lot) => <tr key={String(lot.id)}><td><button className="table-primary" onClick={() => setModal({ type: "detail", lot })}>{String(lot.lot_no)}<small>{String(lot.design_no)} · {String(lot.size_range)}</small></button></td><td>{String(lot.fabrication)}<small className="cell-sub">{String(lot.customer)}</small><small className="cell-sub colour-size-summary" title={lotBreakdownText(lot, state)}>{lotBreakdownText(lot, state)}</small></td><td><b>{fmt(lot.quantity)}</b><small className="cell-sub">PCS</small></td><td><span className="department-chip">{departmentIcon(String(lot.current_department))}{String(lot.current_department)}</span><StatusBadge status={lot.status} /></td><td><b>{fmt(lot.completed_qty)}</b><small className="cell-sub">{fmt(Math.max(0, number(lot.quantity) - number(lot.completed_qty)))} pending</small><Progress value={lotProgress(lot)} compact /></td><td>{formatDate(lot.required_delivery_date)}</td><td><span className="remarks-cell" title={String(lot.remarks)}>{String(lot.remarks)}</span></td><td className="right"><div className="row-actions"><button title="Print full progress sheet" aria-label="Print full progress sheet" onClick={() => printLotBook([lot], state, "Lot Progress Full Sheet")}><Printer size={16} /></button><button title="Edit lot" aria-label="Edit lot" onClick={() => setModal({ type: "edit-lot", lot })}><Pencil size={16} /></button><button title="Add remark" aria-label="Add remark" onClick={() => setModal({ type: "remark", lot })}><FileText size={16} /></button></div></td></tr>)}</tbody></table></div>{!rows.length && <Empty title="No production lots found" detail="Try clearing a filter or search for a different Design No." />}</article>
  </div>;
}
...
// Per-department colour/size entry. Totals are summed here so the department
// never types the same number twice.
type VariantDraft = { colour: string; size: string; received: number; completedQty: string; rejectedQty: string; reworkQty: string };

function VariantProgressGrid({ rows, setRows, label }: { rows: VariantDraft[]; setRows: (next: VariantDraft[]) => void; label: string }) {
  const update = (index: number, key: keyof VariantDraft, value: string) => setRows(rows.map((row, i) => i === index ? { ...row, [key]: value } : row));
  const sum = (key: "completedQty" | "rejectedQty" | "reworkQty") => rows.reduce((total, row) => total + number(row[key]), 0);
  const received = rows.reduce((total, row) => total + row.received, 0);
  const done = sum("completedQty");
  return <div className="variant-grid">
    <div className="variant-grid-head"><span><span className="eyebrow">COLOUR / SIZE-WISE {label.toUpperCase()}</span><p>Enter what each colour and size finished. The department total adds up from these rows.</p></span>
      <button type="button" className="link-button" onClick={() => setRows(rows.map((row) => ({ ...row, completedQty: String(row.received), rejectedQty: "0", reworkQty: "0" })))}>Fill all complete</button></div>
    <div className="variant-table">
      <div className="variant-row variant-header"><span>Colour</span><span>Size</span><span>Received</span><span>{label}</span><span>Rejected</span><span>Rework</span><span>Pending</span></div>
      {rows.map((row, index) => { const pending = Math.max(0, row.received - number(row.completedQty) - number(row.rejectedQty)); return <div className="variant-row" key={`${row.colour}|${row.size}`}>
        <span className="variant-colour"><i style={{ background: swatch(row.colour) }} />{row.colour}</span>
        <span className="variant-size">{row.size}</span>
        <span><b>{fmt(row.received)}</b></span>
        <input type="number" min="0" max={row.received} value={row.completedQty} onChange={(event) => update(index, "completedQty", event.target.value)} aria-label={`${label} for ${row.colour} ${row.size}`} />
        <input type="number" min="0" value={row.rejectedQty} onChange={(event) => update(index, "rejectedQty", event.target.value)} aria-label={`Rejected for ${row.colour} ${row.size}`} />
        <input type="number" min="0" value={row.reworkQty} onChange={(event) => update(index, "reworkQty", event.target.value)} aria-label={`Rework for ${row.colour} ${row.size}`} />
        <span className={pending === 0 ? "green-text" : "orange-text"}><b>{fmt(pending)}</b></span>
      </div>; })}
    </div>
    <div className="variant-total"><span>Received <b>{fmt(received)}</b></span><span>{label} <b className="green-text">{fmt(done)}</b></span><span>Rejected <b className="red-text">{fmt(sum("rejectedQty"))}</b></span><span>Rework <b className="orange-text">{fmt(sum("reworkQty"))}</b></span><span>Pending <b>{fmt(Math.max(0, received - done - sum("rejectedQty")))}</b></span></div>
  </div>;
}

// Stable, readable swatch for a colour name; recognised names use their own hue.
function swatch(colour: string) {
  const known: Record<string, string> = { black: "#1d1f22", white: "#f2f4f3", red: "#cf3b3b", blue: "#3569df", navy: "#1e2f63", green: "#2f9e44", yellow: "#e0b325", orange: "#d98516", pink: "#dd6d9a", purple: "#7753c7", grey: "#8c9a91", gray: "#8c9a91", brown: "#8a5a3b", beige: "#d9c7a7", maroon: "#7b2b3a", cream: "#efe6d2", teal: "#15858a" };
...
function ProductionModal({ lot, department, state, onClose, onSave, saving }: { lot: Row; department: string; state: FactoryState; onClose: () => void; onSave: PageProps["post"]; saving: boolean }) {
  const record = state.records[department]?.find((item) => number(item.lot_id) === number(lot.id)); const received = number(record?.received_qty);
  const myVariants = state.variants.filter((row) => number(row.lot_id) === number(lot.id) && String(row.department) === department);
  const [variants, setVariants] = useState<VariantDraft[]>(myVariants.map((row) => ({ colour: String(row.colour), size: String(row.size), received: number(row.received_qty), completedQty: String(number(row.completed_qty)), rejectedQty: String(number(row.rejected_qty)), reworkQty: String(number(row.rework_qty)) })));
  const byVariant = variants.length > 0;
  const [form, setForm] = useState({ completedQty: String(record?.completed_qty ?? 0), rejectedQty: String(record?.rejected_qty ?? 0), reworkQty: String(record?.rework_qty ?? 0), status: String(record?.status === "Waiting" ? "Received" : record?.status || "Received"), remarks: String(record?.remarks || ""), targetQty: String(record?.target_qty || received), todayProduction: String(record?.today_production || 0), productionLine: String(record?.production_line || "Line 01"), supervisor: String(record?.supervisor || ""), process: String(record?.process || "General Quality Check"), piecesPerCarton: String(record?.pieces_per_carton || 20) }); const [error, setError] = useState("");
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    // With a colour/size grid present the department totals are summed from it,
    // so the same numbers are never typed twice.
    const completed = byVariant ? variants.reduce((sum, row) => sum + number(row.completedQty), 0) : number(form.completedQty);
    const rejected = byVariant ? variants.reduce((sum, row) => sum + number(row.rejectedQty), 0) : number(form.rejectedQty);
    const rework = byVariant ? variants.reduce((sum, row) => sum + number(row.reworkQty), 0) : number(form.reworkQty);
    if ([completed, rejected, rework].some((value) => value < 0)) return setError("Incorrect Quantity — quantities cannot be negative.");
    if (byVariant) {
      const over = variants.find((row) => number(row.completedQty) + number(row.rejectedQty) > row.received);
      if (over) return setError(`${over.colour} / ${over.size}: completed plus rejected cannot exceed the ${fmt(over.received)} PCS received.`);
    } else {
      if (completed > received) return setError("Completed QTY cannot exceed Received QTY.");
      if (completed + rejected > received) return setError("Completed plus Rejected QTY cannot exceed Received QTY.");
    }
    await onSave({ action: "update-production", lotId: lot.id, department, ...form, completedQty: completed, rejectedQty: rejected, reworkQty: rework, targetQty: number(form.targetQty), todayProduction: number(form.todayProduction), piecesPerCarton: number(form.piecesPerCarton), variants: byVariant ? variants.map((row) => ({ colour: row.colour, size: row.size, completedQty: number(row.completedQty), rejectedQty: number(row.rejectedQty), reworkQty: number(row.reworkQty) })) : [] });
  };
  const label = department === "Cutting" || department === "Finishing" ? "Passed QTY" : department === "Packing" ? "Packing QTY" : "Completed QTY";
  return <Modal title={`Update ${department} Production`} subtitle={`${String(lot.design_no)} / ${String(lot.lot_no)} · ${fmt(received)} PCS received`} onClose={onClose} wide><form onSubmit={submit}>{byVariant && <VariantProgressGrid rows={variants} setRows={setVariants} label={label.replace(" QTY","")} />}<div className="production-summary"><span><small>Received QTY</small><b>{fmt(received)}</b></span><span><small>Already transferred</small><b>{fmt(record?.transferred_qty)}</b></span><span><small>Calculated pending</small><b>{fmt(Math.max(0, received - number(form.completedQty)))}</b></span>{department === "Packing" && <span><small>Calculated cartons</small><b>{Math.ceil(number(form.completedQty) / Math.max(1, number(form.piecesPerCarton)))}</b></span>}</div><div className="form-grid three">{!byVariant && <><Field label={label}><input type="number" min="0" max={received} value={form.completedQty} onChange={(e) => { setForm({ ...form, completedQty: e.target.value }); setError(""); }} /></Field><Field label="Rejected QTY"><input type="number" min="0" value={form.rejectedQty} onChange={(e) => setForm({ ...form, rejectedQty: e.target.value })} /></Field><Field label="Rework QTY"><input type="number" min="0" value={form.reworkQty} onChange={(e) => setForm({ ...form, reworkQty: e.target.value })} /></Field></>}{department === "Stitching" && <><Field label="Target QTY"><input type="number" value={form.targetQty} onChange={(e) => setForm({ ...form, targetQty: e.target.value })} /></Field><Field label="Today's Production"><input type="number" value={form.todayProduction} onChange={(e) => setForm({ ...form, todayProduction: e.target.value })} /></Field><Field label="Production Line"><input value={form.productionLine} onChange={(e) => setForm({ ...form, productionLine: e.target.value })} /></Field></>}{department === "Finishing" && <Field label="Finishing Process"><select value={form.process} onChange={(e) => setForm({ ...form, process: e.target.value })}>{["Thread Cutting","Ironing","Measurement Check","Cleaning","Final Inspection","Button / Accessory Check","Label Check","General Quality Check"].map((item) => <option key={item}>{item}</option>)}</select></Field>}{department === "Packing" && <Field label="Pieces Per Carton"><input type="number" min="1" value={form.piecesPerCarton} onChange={(e) => setForm({ ...form, piecesPerCarton: e.target.value })} /></Field>}<Field label="Status"><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>{["Received","In Progress","Running","Partially Completed","Completed","Hold","Rework"].map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Supervisor"><input value={form.supervisor} onChange={(e) => setForm({ ...form, supervisor: e.target.value })} /></Field><Field label="Timestamped Remarks" span><textarea rows={3} value={form.remarks} onChange={(e) => setForm({ ...form, remarks: e.target.value })} placeholder={`Add a ${department.toLowerCase()} production note…`} /></Field></div>{error && <div className="form-alert"><AlertTriangle size={16} />{error}</div>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={saving}>{saving && <LoaderCircle className="spin" size={16} />} Save Production Update</button></div></form></Modal>;
}
