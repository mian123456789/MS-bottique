...
export async function POST(request: Request) {
  try {
    ...
    await env.DB.prepare("INSERT INTO attachments (design_id,file_name,object_key,content_type,size,uploaded_by) VALUES (?,?,?,?,?,1)").bind(design.id,file.name,key,file.type,file.size).run();
    await env.DB.prepare("INSERT INTO audit_logs (user_id,design_id,action,entity,entity_id,new_value,device) VALUES (1,?,'UPLOAD','attachment',?,?, 'Web / Chrome')").bind(design.id,key,file.name).run();
